...
It iterates through image `A`, seeing if a character matches `B[0]`. Then it performs the same search with `A[i+1]` and `B[j+1]`. However, the index `i+1` or `j+1` has to be modified if it reaches the end of the row.
...
def search(i,j) :
    global a,b,n,m
    # if b[j] were to yield an index error, means all characters in b have found a match
    if j >= m**2 :
        return True
