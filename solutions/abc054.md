...
It iterates through image A, seeing if a character matches B[0]. Then it performs the same search with A[i+1] and B[j+1]. However, the index i+1 or j+1 has to be modified if we reach the end of the row.
