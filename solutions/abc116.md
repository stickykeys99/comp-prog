...
If we ignore the variety bonus, we simply need to get the top `K` sushi, maximizing the first part of the score. Now we see, we can deliberately increase the variety bonus one by one, by dropping the lowest from the top `K` that does not have a unique topping group, and choosing the highest-scoring sushi that also contributes to our variety bonus. We do this until we can no longer increase the variety bonus. 
